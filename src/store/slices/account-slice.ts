...
import { NirvanaTokenContract, BlissTokenContract, UsdtTokenContract } from "../../abi";
...
export const getBalances = createAsyncThunk("account/getBalances", async ({ address, networkID, provider }: IGetBalances): Promise<IAccountBalances> => {
    ...
    const blissBalance = await blissContract.balanceOf(address);
    const nirvContract = new ethers.Contract(addresses.NIRV_ADDRESS, NirvanaTokenContract, provider);
    const nirvBalance = await nirvContract.balanceOf(address);

    return {
        balances: {
            bliss: ethers.utils.formatUnits(blissBalance, "gwei"),
            nirv: ethers.utils.formatUnits(nirvBalance, "gwei"),
        },
    };
});
...
export const loadAccountDetails = createAsyncThunk("account/loadAccountDetails", async ({ networkID, provider, address }: ILoadAccountDetails): Promise<IUserAccountDetails> => {
    ...
    if (addresses.NIRV_ADDRESS) {
        const nirvContract = new ethers.Contract(addresses.NIRV_ADDRESS, NirvanaTokenContract, provider);
        nirvBalance = await nirvContract.balanceOf(address);
        stakeAllowance = await nirvContract.allowance(address, addresses.STAKING_HELPER_ADDRESS);
    }

    if (addresses.BLISS_ADDRESS) {
        const blissContract = new ethers.Contract(addresses.BLISS_ADDRESS, BlissTokenContract, provider);
        blissBalance = await blissContract.balanceOf(address);
        unstakeAllowance = await blissContract.allowance(address, addresses.STAKING_ADDRESS);
    }
    ...
    return {
        ...
        staking: {
            nirv: Number(stakeAllowance),
            bliss: Number(unstakeAllowance),
        },
    };
});
...
export const calculateUserTokenDetails = createAsyncThunk("account/calculateUserTokenDetails", async ({ address, token, networkID, provider }: ICalcUserTokenDetails) => {
    ...
    const tokenContract = new ethers.Contract(token.address, UsdtTokenContract, provider);
